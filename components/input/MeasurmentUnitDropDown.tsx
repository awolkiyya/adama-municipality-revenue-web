...
interface MeasurementUnitDropdownProps {
  value: string | null;
  onChange: (value: string, item: MeasurementUnit) => void;
  disabled?: boolean;
}

export const MeasurementUnitDropdown: React.FC<MeasurementUnitDropdownProps> = ({
  value,
  onChange,
  disabled = false,
}) => {
  ...
  const fetchMeasurementUnits = useCallback(
    async ({
      ...
      const result = await measurementUnitService.getUnits({
        search,
        page,
        per_page: pageSize,
        isActive: true,
  ...
  return (
    <AsyncDropdown<MeasurementUnit, string>
      value={value}
      onChange={onChange}
